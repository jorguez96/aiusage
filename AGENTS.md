...
- Cross-device usage merges at the record level with `aiusage sync` (S3 backend) against a local s3rver store on the Windows side (`C:\Users\jorgu\.aiusage\s3store`, port 9000; WSL reaches it via the Hyper-V host gateway IP). Serve topology: Linux `:45680` from the fork checkout reads `~/.aiusage/cache.db`; Windows `:3847` serves the fork dist (`C:\Users\jorgu\aiusage-fork`, reads `C:\Users\jorgu\.aiusage\cache.db`) through the hermes node binary, kept up probe-first by the same 15-min task chain (`AIUsage-WSL-Sync` → `wsl-sync.cmd` / `wsl-sync.sh` → `firstmate/config/aiusage-start.sh`), so a scheduled run never spawns a duplicate serve; rebuild that dist with Windows-side pnpm (hermes node install), never WSL `pnpm build` (that checkout's `node_modules` carries Windows-only native optionals). If :3847 ever stacks servers again, suspect `aiusage-start.sh`'s DIST pointing upstream (was the 2026-09-24 incident: fixed by switching DIST to the fork) combined with `serve.ts` rolling to the next port on EADDRINUSE; the tick's stop only matches `*@juliantanx*aiusage*` command lines, so fork-dist serves are never reaped by the tick (probe-first just leaves the holder alone). The 15-min `AIUsage-WSL-Sync` task runs parse+sync on both sides (`wsl-sync.cmd` / `wsl-sync.sh`). Never re-enable session-log rsync across machines: sync ids are per-device, so dual-parsed sessions double-count after merge. Never point the scheduled parse at the upstream package build — only fork builds carry the sync provenance fix. Quota bridge stays on its ~5 min schedule, untouched by sync.
